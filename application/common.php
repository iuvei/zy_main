 ...
 * @return mixed
 */
function paginate_data ($page, $per_page, $params, $where, $list, $url, $model, $method, $group = null)
{
    ...
    // 总条数
    $sum = $model::where($where)->group($group)->count();
...
function compare_bet_money($class, $odds, $bet, $order, $odds_id, $mark_b, &$expect_orders)
{
    ...
    if ($bet['money'] < $order_limit_min) {
        $jsonData['status'] = 0;
        $jsonData['msg'] = "[ " . $order['mark_a'] . " ][ " . $order['mark_b'] . " ]下注金额不足单注最低限额";
        return $jsonData;
    }
    if ($bet['money'] > $order_limit_max) {
        $jsonData['status'] = 0;
        $jsonData['msg'] = "[ " . $order['mark_a'] . " ][ " . $order['mark_b'] . " ]下注金额超过单注最高限额";
        return $jsonData;
    }
    ...
    if ($betedSumMoney + $expect_orders[$odds_id]['sum_money'] > $expect_limit) {
        $jsonData['status'] = 0;
        $jsonData['msg'] = $bet['expect'] . "期 [ " . $order['mark_a'] . " ][ " . $order['mark_b'] . " ]单期下注金额超过单期最高限额";
        return $jsonData;
    }
    return true;
}