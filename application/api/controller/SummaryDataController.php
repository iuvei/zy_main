...
class SummaryDataController extends BaseController
{
    ...
    public function clearList()
    {
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            // 查询报表
            $sum = $this->calcWithTime($start, $end);
        ...
        // 本月
        $thisMonthSum = [];
        $thisMonthRange = get_time_range('this_month');
        $monthStart = $thisMonthRange['start'];
        $monthEnd = $thisMonthRange['end'];
        $count = 0;
        for ($start = $monthStart; $start < $monthEnd; $start = $start + 86400) {
            $end = $start + 86400;
            $thisMonthSum[$count]['date_str'] = date('Y-m-d', $start);
            // 查询报表
            $sum = $this->calcWithTime($start, $end);
            $thisMonthSum[$count]['sum_data'] = $sum;
            $count++;
        }

        $this->jsonData['data']['last_week'] = $lastWeekSum;
        $this->jsonData['data']['this_week'] = $thisWeekSum;
        $this->jsonData['data']['this_month'] = $thisMonthSum;
     ...
     * @throws \think\exception\DbException
     */
    public function clearList2(Request $request)
    {
        // 指定日期范围
        $param = $request->only(['range'], 'get');
        $error = $this->validate($param, [
            'range|查询区间'        => 'alphaDash'
        ]);
        if (true !== $error) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg'] = $error;
        ...
        $range = get_time_range($param['range']);
        $rangeSum = [];
        $count = 0;
        for ($start = $range['start']; $start < $range['end']; $start = $start + 86400) {
            $end = $start + 86400;
            $rangeSum[$count]['date_str'] = date('Y-m-d', $start);
            // 查询报表
            $sum = $this->calcWithTime($start, $end);
            $rangeSum[$count]['sum_data'] = $sum;
            $count++;
        }

        $this->jsonData['data']['range'] = $rangeSum;
     ...
     * @throws \think\exception\DbException
     */
    private function calcWithTime($start, $end)
    {
    ...
    {
        $get = $request->only(['date', 'page', 'per_page'], 'get');
        ...
        $url = $request->baseUrl();
        $orders = $this->oneDayOrders($get, $url);
     ...
     * @throws \think\exception\DbException
     */
    private function oneDayOrders($get, $url)
    {
        $date = $get['date'];
