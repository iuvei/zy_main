<?php
/**
 * Created by PhpStorm.
 * User: fish
 * Date: 2018/3/7
 * Time: 9:47
 */

namespace app\cqssc\controller;


use app\api\model\AccMoney;
use app\api\model\AccMychg;
use app\api\model\AccUsers;
use app\auth\controller\BaseController;
use app\cqssc\model\SscCustomOdds;
use app\cqssc\model\SscOdds;
use app\cqssc\model\SscOrder;
use app\cqssc\model\SscRatelist;
use app\cqssc\model\SscUser;
use GatewayClient\Gateway;
use GuzzleHttp\Client;
use think\Config;
use think\helper\Time;
use think\Request;

class SscOrderController extends BaseController
{
    protected function _initialize()
    {
        parent::_initialize(); // TODO: Change the autogenerated stub
        Config::load(APP_PATH . 'cqssc/config.php');
    }

    /**
     * @param Request $request
     *
     * @return array
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\ModelNotFoundException
     * @throws \think\exception\DbException
     */
    public function save(Request $request)
    {
        $post = $request->only([
            'bets',
            'odds_table'
        ], 'post');

        $error = $this->validate($post, [
            'bets'       => 'require',
            'odds_table' => 'require',
        ]);
        if (true !== $error) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg']    = $error;
            return $this->jsonData;
        }

        // 时时彩当前闭盘、开奖时间、期数
        $sscTime = get_cur_timelist('cqssc');
        // 比对时间，闭盘则不能下注
        if ($sscTime['endtime'] <= 0) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg']    = $sscTime['expect'] . '期已闭盘，下注失败！';
            return $this->jsonData;
        }
        $create_time = time();
        $update_time = $create_time;

        $oddsTable   = 'ssc_' . trim($post['odds_table']);
        $existTables = SscRatelist::where(['tokenint' => $this->user->tokenint])->column('ratewin_set');
        if (empty($existTables)) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg']    = '没有权限下注';
            return $this->jsonData;
        } else if (!in_array($oddsTable, $existTables)) {
            $oddsTable = SscRatelist::where([
                'tokenint' => $this->user->tokenint,
                'sel'      => 1
            ])->value('ratewin_set');
            if (!$oddsTable) {
                $oddsTable = $existTables[0];
            }
        }

        $bets = $post['bets'];
        // 计算总共下注多少钱，与余额及信用额度比较，不足则下注失败
        $sumBetMoney = array_sum(array_column($bets, 'money'));
        $userMoney   = AccMoney::getMoneyByUserid($this->user->id);
        if ($sumBetMoney > ($userMoney->cash_money + $userMoney->credit_money)) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg']    = '下注失败，您的余额与信用额度不足，请先充值';
            return $this->jsonData;
        }
        $orders         = [];
        $order_nos      = [];
        $websocket_data = [];
        $expect_orders  = [];
        foreach ($bets as $key => $bet) {
            $bet['expect'] = $sscTime['expect'];
            $order = $this->generateOrderData($expect_orders, $bet, $oddsTable);
            if (count($order) <= 3) {
                return $order;
            }
            $order['expect'] = $sscTime['expect'];
            // 订单状态 0下注成功
            $order['status']      = 0;
            $order['create_time'] = $create_time;
            $order['update_time'] = $update_time;
            array_push($orders, $order);
            array_push($order_nos, $order['order_no']);

            // 组装数据发送给前端页面
            $websocket_data[$key]['nickname']    = $order['nickname'];
            $websocket_data[$key]['lty_name']    = $order['lty_name'];
            $websocket_data[$key]['mark_a']      = $order['mark_a'];
            $websocket_data[$key]['mark_b']      = $order['mark_b'];
            $websocket_data[$key]['money']       = $order['money'];
            $websocket_data[$key]['create_time'] = $order['create_time'];
            $websocket_data[$key]['order_no']    = $order['order_no'];
            $websocket_data[$key]['expect']      = $order['expect'];
            $websocket_data[$key]['rate']        = $order['rate'];
            $websocket_data[$key]['win']         = $order['win'];
            $websocket_data[$key]['fs_sv']       = $order['fs_sv'];
            $websocket_data[$key]['fs_av']       = $order['fs_av'];
            $websocket_data[$key]['fs_mv']       = $order['fs_mv'];
            $websocket_data[$key]['fs_gv']       = $order['fs_gv'];
            $websocket_data[$key]['ls_sv']       = $order['ls_sv'];
            $websocket_data[$key]['ls_av']       = $order['ls_av'];
            $websocket_data[$key]['ls_mv']       = $order['ls_mv'];
            $websocket_data[$key]['ls_gv']       = $order['ls_gv'];
        }

        $orderModel = new SscOrder();
        $res        = $orderModel->saveAll($orders);
        if ($res) {
            // 增加资金变动
            $user                       = $this->user;
            $chg_column['tokenint']     = $user->tokenint;
            $chg_column['username']     = $user->username;
            $chg_column['nickname']     = $user->nickname;
            $chg_column['c_type']       = CHG_BET;
            $chg_column['c_old']        = $userMoney->cash_money;
            $chg_column['chg']          = $sumBetMoney;
            $chg_column['cur']          = $chg_column['c_old'] - $chg_column['chg'];
            $chg_column['con']          = '重庆时时彩下注';
            $chg_column['opr_tokenint'] = '';
            $chg_column['opr_nickname'] = '';
            $chg_column['opr_username'] = '';
            $chg_column['opr_ip']       = request()->ip();
            $chg_column['opr_time']     = get_cur_date();
            $chg_column['opr_mark']     = '订单号：' . implode(',', $order_nos);
            $chg_id                     = add_chg($chg_column);

            $chg = AccMychg::get($chg_id);
            // 更新用户余额
            upd_money($chg->tokenint, $chg->cur);

            $client_ids = get_all_admin_client_ids($user);
            $msg        = [
                'type'     => 'orders',
                'msg'      => $websocket_data,
                'lty_name' => 'cqssc',
            ];
            file_put_contents('socket.log', implode(',', $client_ids));
            Gateway::sendToAll(json_encode($msg), $client_ids);

            $this->jsonData['msg'] = '下注成功';
            return $this->jsonData;
        }
        $this->jsonData['msg'] = '下注失败';
        return $this->jsonData;
    }

    /**
     * @param Request $request
     *
     * @return array
     */
    public function save2(Request $request)
    {
        try {
            $post = $request->post();
            if (!isset($post['tokensup'])) {
                $this->jsonData['status'] = 0;
                $this->jsonData['msg']    = '非法请求';
                return $this->jsonData;
            }
            $user = AccUsers::getUserByTokensup($post['tokensup']);
            if (is_null($user)) {
                $this->jsonData['status'] = 0;
                $this->jsonData['msg']    = '参数错误';
                return $this->jsonData;
            }

            // 时时彩当前闭盘、开奖时间、期数
            $sscTime = get_cur_timelist('cqssc');
            // 比对时间，闭盘则不能下注
            if ($sscTime['endtime'] <= 0) {
                $this->jsonData['status'] = 0;
                $this->jsonData['msg']    = $sscTime['expect'] . '期已闭盘，下注失败！';
                return $this->jsonData;
            }
            $create_time = time();
            $update_time = $create_time;

            $bets = $post['bets'];
            // 计算总共下注多少钱，与余额及信用额度比较，不足则下注失败
            $sumBetMoney = array_sum(array_column($bets, 'money'));
            $userMoney   = AccMoney::getMoneyByUserid($user->id);
            if ($sumBetMoney > ($userMoney->cash_money + $userMoney->credit_money)) {
                $this->jsonData['status'] = 0;
                $this->jsonData['msg']    = '下注失败，您的余额与信用额度不足，请先充值';
                return $this->jsonData;
            }
            $orders         = [];
            $order_nos      = [];
            $websocket_data = [];
            $expect_orders  = [];
            foreach ($bets as $key => $bet) {
                $bet['expect'] = $sscTime['expect'];
                $order = $this->generateOrderData($expect_orders, $bet, null, $user->id);
                if (count($order) <= 3) {
                    return $order;
                }
                $order['expect'] = $sscTime['expect'];
                // 订单状态 0下注成功
                $order['status']      = 0;
                $order['trad_stu']    = 1;
                $order['create_time'] = $create_time;
                $order['update_time'] = $update_time;
                array_push($orders, $order);
                array_push($order_nos, $order['order_no']);

                // 组装数据发送给前端页面
                $websocket_data[$key]['nickname']    = $order['nickname'];
                $websocket_data[$key]['lty_name']    = $order['lty_name'];
                $websocket_data[$key]['mark_a']      = $order['mark_a'];
                $websocket_data[$key]['mark_b']      = $order['mark_b'];
                $websocket_data[$key]['money']       = $order['money'];
                $websocket_data[$key]['create_time'] = $order['create_time'];
                $websocket_data[$key]['order_no']    = $order['order_no'];
                $websocket_data[$key]['expect']      = $order['expect'];
                $websocket_data[$key]['rate']        = $order['rate'];
                $websocket_data[$key]['win']         = $order['win'];
                $websocket_data[$key]['fs_sv']       = $order['fs_sv'];
                $websocket_data[$key]['fs_av']       = $order['fs_av'];
                $websocket_data[$key]['fs_mv']       = $order['fs_mv'];
                $websocket_data[$key]['fs_gv']       = $order['fs_gv'];
                $websocket_data[$key]['ls_sv']       = $order['ls_sv'];
                $websocket_data[$key]['ls_av']       = $order['ls_av'];
                $websocket_data[$key]['ls_mv']       = $order['ls_mv'];
                $websocket_data[$key]['ls_gv']       = $order['ls_gv'];
            }

            $orderModel = new SscOrder();
            $res        = $orderModel->saveAll($orders);
            if ($res) {
                // 增加资金变动
                $chg_column['tokenint']     = $user->tokenint;
                $chg_column['username']     = $user->username;
                $chg_column['nickname']     = $user->nickname;
                $chg_column['c_type']       = CHG_BET;
                $chg_column['c_old']        = $userMoney->cash_money;
                $chg_column['chg']          = $sumBetMoney;
                $chg_column['cur']          = $chg_column['c_old'] - $chg_column['chg'];
                $chg_column['con']          = '重庆时时彩下注';
                $chg_column['opr_tokenint'] = '';
                $chg_column['opr_nickname'] = '';
                $chg_column['opr_username'] = '';
                $chg_column['opr_ip']       = request()->ip();
                $chg_column['opr_time']     = get_cur_date();
                $chg_column['opr_mark']     = '订单号：' . implode(',', $order_nos);
                $chg_id                     = add_chg($chg_column);

                $chg = AccMychg::get($chg_id);
                // 更新用户余额
                upd_money($chg->tokenint, $chg->cur);

                $client_ids = get_all_admin_client_ids($user);
                $msg        = [
                    'type'     => 'orders',
                    'msg'      => $websocket_data,
                    'lty_name' => 'cqssc',
                ];
                Gateway::sendToAll(json_encode($msg), $client_ids);

                $this->jsonData['msg'] = '下注成功';
                return $this->jsonData;
            }
            $this->jsonData['msg'] = '下注失败';
            return $this->jsonData;
        } catch (\Exception $e) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg'] = $e->getMessage();
            return $this->jsonData;
        }
    }

    /**
     * @param Request $request
     *
     * @return array
     * @throws \think\exception\DbException
     */
    public function history(Request $request)
    {
        $params   = $request->only(['page', 'per_page', 'clear', 'lucky', 'range']);
        $tokenint = $this->user->tokenint;
        // 页数
        $page = isset($params['page']) ? $params['page'] : 1;
        if ($page < 1) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg']    = '页码不能小于1';
            return $this->jsonData;
        }
        // 每页显示数量
        $per_page = isset($params['per_page']) ? $params['per_page'] : 15;

        // 查询条件
        $where['tokenint'] = $tokenint;
        if (isset($params['lucky'])) {
            $where['open_ret'] = $params['lucky'];
        }
        if (isset($params['clear'])) {
            $where['open_stu'] = $params['clear'];
        }
        if (isset($params['range'])) {
            $timeRange = get_time_range($params['range']);
            $where['create_time'] = ['between' , [$timeRange['start'], $timeRange['end']]];
        }

        // 下注列表
        $list = SscOrder::history($page, $per_page, $where);
        $url  = $request->baseUrl();

        $data = paginate_data($page, $per_page, $params, $where, $list, $url, SscOrder::class, 'history');

        $this->jsonData['data']   = $data;

        return $this->jsonData;
    }

    /**
     * @param Request $request
     *
     * @return mixed
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\ModelNotFoundException
     * @throws \think\exception\DbException
     */
    public function summary(Request $request)
    {
        $params   = $request->param();
        $tokenint = $this->user->tokenint;
        // 页数
        $page = isset($params['page']) ? $params['page'] : 1;
        if ($page < 1) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg']    = '页码不能小于1';
            return $this->jsonData;
        }
        // 每页显示数量
        $per_page = isset($params['per_page']) ? $params['per_page'] : 15;

        // 查询条件
        $where['tokenint'] = $tokenint;
        $type              = summary_cond($params, $where);
        $list              = SscOrder::summary($page, $per_page, $where);

        foreach ($list as &$item) {
            $item['detail_url'] = url('/ssc/detail', [
                'expect' => $item['expect'],
                'type'   => $type
            ]);
        }

        $url  = $request->baseUrl();
        $data = paginate_data($page, $per_page, $params, $where, $list, $url, SscOrder::class, 'summary', 'lty_name, expect');

        $this->jsonData['data']   = $data;

        return $this->jsonData;
    }

    /**
     * @param Request $request
     *
     * @return mixed
     * @throws \think\exception\DbException
     */
    public function detail(Request $request)
    {
        $params   = $request->param();
        $tokenint = $this->user->tokenint;
        // 页数
        $page = isset($params['page']) ? $params['page'] : 1;
        if ($page < 1) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg']    = '页码不能小于1';
            return $this->jsonData;
        }
        // 每页显示数量
        $per_page = isset($params['per_page']) ? $params['per_page'] : 15;

        // 查询条件
        $where['tokenint'] = $tokenint;
        if (!isset($params['expect'])) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg']    = '缺少 expect 参数';
            return $this->jsonData;
        }
        $where['expect'] = $params['expect'];
        $list            = SscOrder::history($page, $per_page, $where);
        $url             = $request->baseUrl();
        $data            = paginate_data($page, $per_page, $params, $where, $list, $url, SscOrder::class, 'history');

        $this->jsonData['data']   = $data;

        return $this->jsonData;
    }

    /**
     * 组织生成订单信息
     *
     * @param $expect_orders
     * @param      $bet
     * @param      $panTable
     * @param null $user_id
     *
     * @return mixed
     * @throws \think\db\exception\BindParamException
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\ModelNotFoundException
     * @throws \think\exception\DbException
     * @throws \think\exception\PDOException
     */
    private function generateOrderData(&$expect_orders, $bet, $panTable = null, $user_id = null)
    {
        if (is_null($user_id)) {
            $user = $this->user;
        } else {
            $user = AccUsers::get($user_id);
        }
        $order['username'] = $user->username;
        $order['nickname'] = $user->nickname;
        $order['tokenint'] = $user->tokenint;
        $order['money']    = $bet['money'];

        // 检查用户注额设置
        $userTrad = SscUser::getByUser($user->tokenint);
        if ($bet['money'] < $userTrad->money_min) {
            return [
                'status' => 0,
                'msg'    => '下注失败，单注下注额不足最小下注限额'
            ];
        }
        if ($bet['money'] > $userTrad->money_max) {
            return [
                'status' => 0,
                'msg'    => '下注失败，单注下注额超出最大下注限额'
            ];
        }

        $order['order_no'] = order_no();
        usleep(mt_rand(1, 10));
        $order['lty_name'] = '重庆时时彩';

        // 解析玩法和下注内容
        $content         = $this->handleKey(trim($bet['content']));
        $type            = $content['type'];
        $column          = $content['column'];
        $mark_b          = \config('ssc_type')[$type];
        $order['mark_a'] = $mark_b;
        $order['mark_b'] = $this->getBetContent($type, $column);

        // 赔率
        $odds_id = $this->getOddsId($mark_b);
        if ($odds_id === 0) {
            return [
                'status' => 0,
                'msg'    => '下注失败！'
            ];
        }
        if (is_null($panTable)) {
            $pan      = SscRatelist::getPanByUser($user->tokenint);
            $panTable = 'ssc_' . strtolower($pan);
        }

        // 检查用户该赔率表是否有定制赔率
        $customExists = SscCustomOdds::isExists($panTable, $user->tokenint);
        if ($customExists) {
            $odds = SscCustomOdds::getOddsByTableAndUser($panTable, $user->tokenint);
        } else {
            $odds = SscOdds::getAll($panTable)->toArray();
        }

        $result = compare_bet_money(SscOrder::class, $odds, $bet, $order, $odds_id, $mark_b, $expect_orders);
        if (true !== $result) {
            return $result;
        }

        $rate = $odds[$odds_id - 1][$column];
        //        $rate = SscOdds::getRate($panTable, $odds_id, $column);
        $order['rate'] = $rate;
        $win           = $rate * $bet['money'];
        if ($win > $userTrad->money_win) {
            return [
                'status' => 0,
                'msg'    => '下注失败！预赢金额超标'
            ];
        }
        $order['win'] = $win;

        // 各级返点和返水

        //        $fs_g = SscOdds::getFs($panTable, 1);
        //        $fs_m = SscOdds::getFs($panTable, 2);
        //        $fs_a = SscOdds::getFs($panTable, 3);
        //        $fs_s = SscOdds::getFs($panTable, 4);
        $fs_g           = $odds[0]['A'];
        $fs_m           = $odds[1]['A'];
        $fs_a           = $odds[2]['A'];
        $fs_s           = $odds[3]['A'];
        $order['fs_g']  = $fs_g;
        $order['fs_m']  = $fs_m;
        $order['fs_a']  = $fs_a;
        $order['fs_s']  = $fs_s;
        $fs_gv          = $fs_g * $order['money'];
        $fs_mv          = $fs_m * $order['money'];
        $fs_av          = $fs_a * $order['money'];
        $fs_sv          = $fs_s * $order['money'];
        $order['fs_gv'] = $fs_gv;
        $order['fs_mv'] = $fs_mv;
        $order['fs_av'] = $fs_av;
        $order['fs_sv'] = $fs_sv;

        // 各级流水

        //        $ls_g = SscOdds::getLs($panTable, 1);
        //        $ls_m = SscOdds::getLs($panTable, 2);
        //        $ls_a = SscOdds::getLs($panTable, 3);
        //        $ls_s = SscOdds::getLs($panTable, 4);
        $ls_g           = $odds[0]['B'];
        $ls_m           = $odds[1]['B'];
        $ls_a           = $odds[2]['B'];
        $ls_s           = $odds[3]['B'];
        $order['ls_g']  = $ls_g;
        $order['ls_m']  = $ls_m;
        $order['ls_a']  = $ls_a;
        $order['ls_s']  = $ls_s;
        $ls_gv          = $ls_g * $order['money'];
        $ls_mv          = $ls_m * $order['money'];
        $ls_av          = $ls_a * $order['money'];
        $ls_sv          = $ls_s * $order['money'];
        $order['ls_gv'] = $ls_gv;
        $order['ls_mv'] = $ls_mv;
        $order['ls_av'] = $ls_av;
        $order['ls_sv'] = $ls_sv;

        // 开奖号码
        $order['open_code'] = '';
        // 输赢额
        $order['open_win'] = 0;
        // 是否结算
        $order['open_stu'] = 0;
        // 是否中奖
        $order['open_ret'] = 0;

        // 删除一些不需要的字段
        unset($order['content']);

        return $order;
    }

    /**
     * 根据玩法获取赔率表对应id
     *
     * @param $mark_b
     *
     * @return int
     */
    private function getOddsId($mark_b)
    {
        switch ($mark_b) {
            case '第一球':
                $id = 5;
                break;
            case '第二球':
                $id = 6;
                break;
            case '第三球':
                $id = 7;
                break;
            case '第四球':
                $id = 8;
                break;
            case '第五球':
                $id = 9;
                break;
            case '总和、龙虎':
                $id = 10;
                break;
            case '前三':
                $id = 11;
                break;
            case '中三':
                $id = 12;
                break;
            case '后三':
                $id = 13;
                break;
            default:
                $id = 0;
        }
        return $id;
    }

    /**
     * 拆分字段为玩法和下注内容
     *
     * @param $content
     *
     * @return array
     */
    private function handleKey($content)
    {
        $res         = explode('__', $content);
        $type        = $res[0];
        $bet_content = $res[1];
        return [
            'type'   => $type,
            'column' => $bet_content
        ];
    }

    /**
     * 根据玩法和字段来获取下注选项
     *
     * @param $type
     * @param $column
     *
     * @return mixed
     */
    private function getBetContent($type, $column)
    {
        $arr = [];
        switch ($type) {
            case 'ball_1_digit':
            case 'ball_2_digit':
            case 'ball_3_digit':
            case 'ball_4_digit':
            case 'ball_5_digit':
                $arr = [
                    'A' => '0',
                    'B' => '1',
                    'C' => '2',
                    'D' => '3',
                    'E' => '4',
                    'F' => '5',
                    'G' => '6',
                    'H' => '7',
                    'I' => '8',
                    'J' => '9',
                ];
                break;
            case 'ball_1_half':
            case 'ball_2_half':
            case 'ball_3_half':
            case 'ball_4_half':
            case 'ball_5_half':
                $arr = [
                    'K' => '大',
                    'L' => '小',
                    'M' => '单',
                    'N' => '双',
                ];
                break;
            case 'dragon_and_tiger':
                $arr = [
                    'A' => '总和大',
                    'B' => '总和小',
                    'C' => '总和单',
                    'D' => '总和双',
                    'E' => '龙',
                    'F' => '虎',
                    'G' => '和',
                ];
                break;
            case 'front_3':
            case 'medium_3':
            case 'end_3':
                $arr = [
                    'A' => '豹子',
                    'B' => '顺子',
                    'C' => '对子',
                    'D' => '半顺',
                    'E' => '杂六',
                ];
                break;
            default:
                break;
        }
        $bet_content = $arr[$column];
        return $bet_content;
    }

    /**
     * 取消注单
     * @param Request $request
     * @return mixed
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\ModelNotFoundException
     * @throws \think\exception\DbException
     */
    public function delete(Request $request)
    {
        $param = $request->only(['ids'], 'delete');
        $ids = $param['ids'];
        $success = [];
        $fail = [];
        foreach ($ids as $id) {
            $result = $this->cancelOrder($id);
            if ($result['code'] == 1) {
                array_push($success, $id);
            } else {
                array_push($fail, $id);
            }
        }
        $this->jsonData['data']['success'] = $success;
        $this->jsonData['data']['fail'] = $fail;
        $this->jsonData['data']['msg'] = $result;
        return $this->jsonData;
    }

    /**
     * 处理取消注单
     * @param $id
     * @return array
     * @throws \think\db\exception\DataNotFoundException
     * @throws \think\db\exception\ModelNotFoundException
     * @throws \think\exception\DbException
     */
    private function cancelOrder($id)
    {
        $order = SscOrder::get($id);
        // 判断是否可以取消该订单
        // 0. 已经开奖的不能取消
        if (0 != $order->status) {
            return ['code' => 0, 'msg' => '订单已开奖，不能取消'];
        }
        // 1. 过了封盘时间不能取消
        $sscTime = new SscTimelistController;
        $time = $sscTime->time();
        if ($time['expect'] > $order->expect) {
            return ['code' => 0, 'msg' => '过了封盘时间，不能取消'];
        } else {
            if ($time['endtime'] <= 0) {
                return ['code' => 0, 'msg' => '过了封盘时间，不能取消'];
            }
        }

        // 增加资金变动
        $userMoney   = AccMoney::getMoneyByUserid($this->user->id);
        $user                       = $this->user;
        $chg_column['tokenint']     = $user->tokenint;
        $chg_column['username']     = $user->username;
        $chg_column['nickname']     = $user->nickname;
        $chg_column['c_type']       = CANCEL_ORDER;
        $chg_column['c_old']        = $userMoney->cash_money;
        $chg_column['chg']          = $order['money'];
        $chg_column['cur']          = $chg_column['c_old'] + $chg_column['chg'];
        $chg_column['con']          = '重庆时时彩取消注单';
        $chg_column['opr_tokenint'] = '';
        $chg_column['opr_nickname'] = '';
        $chg_column['opr_username'] = '';
        $chg_column['opr_ip']       = request()->ip();
        $chg_column['opr_time']     = get_cur_date();
        $chg_column['opr_mark']     = '订单号：' . $order->order_no;
        $chg_id                     = add_chg($chg_column);
        $chg = AccMychg::get($chg_id);
        // 更新用户余额
        upd_money($chg->tokenint, $chg->cur);

        // 更新订单状态
        $order->status = -1;
        $order->open_stu = 1;
        $order->isUpdate()->save();
        return ['code' => 1, 'msg' => '取消订单成功'];
    }
}