...
class SscOrderController extends AdminBaseController
{
    /**
     * 用户下注历史
     *
    ...
    public function history(Request $request)
    {
        ...
        // 下注列表
        $list = SscOrder::history($page, $per_page, $where);
        $url  = $request->baseUrl();

        $data = paginate_data($page, $per_page, $params, $where, $list, $url, SscOrder::class, 'history');
    ...
    public function summaryMarkb(Request $request)
    {
        ...
        if (isset($param['expect'])) {
            if (!SscLottery::expectExists($param['expect'])){
                $this->jsonData['status'] = 0;
        ...
        // 查询所有有下注的玩法统计数据
        $markb = SscOrder::getMarkbOrders($where);
        // 查询所有玩法
    ...
    {
        $odds = SscOdds::getRealOdds()->toArray();
        $newOdds = [];
