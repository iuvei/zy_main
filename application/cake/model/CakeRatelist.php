...
class CakeRatelist extends Base
{
    ...
    public static function addRate($post)
    {
        ...
        $ratewin_set = 'cake_' . strtolower(trim($post['ratewin_name']));
    ...
    public static function checkRateDuplicate($post)
    {
        ...
        $ratewin_set = 'cake_' . strtolower($ratewin_name);
    ...
    {
        $exists_tables = exists_cake_odds_table();
        $pan = self::where([
            'tokenint' => $tokenint,
            'sel'      => 1
        ])->find();
