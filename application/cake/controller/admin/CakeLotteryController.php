...
use app\cake\controller\KaijiangController;
use app\auth\controller\AdminBaseController;

class CakeLotteryController extends AdminBaseController
{
    /**
     * @return array
    ...
    {
        $timelist = CakeTimelist::where('draw', '<', date('H:i:s', time()))->select();
        $expects = [];
        $lty = CakeLottery::getLastest();
        $expect = $lty->expect;
    ...
    public function history(Request $request)
    {
        ...
        foreach ($list as $key => &$item) {
            ...
            $item['details'] = $this->handleDetails($kaijiangCtrl->getCakeResult($item['opencode']));
    ...
    public function manLottery(Request $request)
    {
        ...
        // 检查期数是否存在
        if (!CakeLottery::expectExists($expect)) {
        ...
        // 检查是否已经开奖的
        if (CakeLottery::expectLotteried($expect)) {
            $this->jsonData['status'] = 0;
            $this->jsonData['msg'] = "加拿大28 [ " . $expect . " ]  已经开奖";
            return $this->jsonData;
        }
        // 开奖结果
        $result = $this->cakeResult($open_codes);
        // 兑奖
        $lastest['expect'] = $expect;
        $lastest['open_code'] = $open_code;
        $msg = $this->setLucky($lastest, $result);
        // 更新开奖状态
        $lottery             = CakeLottery::getByExpect($expect);
        $lottery->is_lottery = 1;
        $lottery->save();

        $this->jsonData['msg'] = "加拿大28 [ " . $expect . " ]  手动开奖成功";
        return $this->jsonData;
    }
    ...
    private function setLucky($latest, $result)
    {
        ...
        $config = config('cake_open_type');

        // 获取所有本期未开奖注单
        $orders     = CakeOrder::getNotOpenOrdersByExpect($expect);
     ...
     * @return array
     */
    public function getCakeResult($open_code)
    {
        return $this->cakeResult($open_code);
    }
     ...
     * @return array
     */
    private function cakeResult($open_codes)
    {
        $ret = [];
        ...
        $ret['ball_0'] = [$firstNum, $secondNum, $thirdNum];
