...
class CakeTradlistController extends AdminBaseController
{
    ...
    public function save(Request $request)
    {
        ...
        if (!$trad_id) {
            return ['status' => 0, 'msg' => '添加转盘失败'];
        }

        $theTrad = CakeTradlist::get($trad_id);
        return ['status' => 200, 'msg' => '添加转盘成功', 'data' => [
            'trad' => $theTrad
        ]];
    }
    ...
    private function checkPost($post)
    {
        if (!isset($post['trad_name']) || '' == trim($post['trad_name'])) {
            exit(json_encode(['status' => 0, 'msg' => '转盘名称不能为空']));
        }
        if (!isset($post['trad_tokensup']) || '' == trim($post['trad_tokensup'])) {
            exit(json_encode(['status' => 0, 'msg' => '转盘密钥不能为空']));
        }
        if (!isset($post['trad_url']) || '' == trim($post['trad_url'])) {
            exit(json_encode(['status' => 0, 'msg' => '转盘接口地址不能为空']));
        }
        ...
        if (CakeTradlist::checkDuplicate(trim($post['trad_tokensup']), trim($post['trad_name']))) {
            exit(json_encode(['status' => 0, 'msg' => '请匆重复添加']));
        }
    }
    ...
    public function update(Request $request, $id)
    {
        $put = $request->put();
        if (empty($put)) {
            return ['status' => 0, 'msg' => '修改失败，参数不能为空'];
        }
        if (isset($put['trad_tokensup']) && isset($put['trad_name']) &&
            CakeTradlist::checkDuplicate(trim($put['trad_tokensup']), trim($put['trad_name']))) {
            return ['status' => 0, 'msg' => '修改失败，重复的名称或密钥'];
        }
        ...
        $trad = CakeTradlist::get($id);
        return ['status' => 200, 'msg' => '修改成功', 'data' => [
            'trad' => $trad
        ]];
    }
    ...
    public function delete($id)
    {
        ...
        if ($result) {
            return ['status' => 200, 'msg' => '删除记录成功'];
        }
        return ['status' => 0, 'msg' => '删除记录失败'];
    }
}