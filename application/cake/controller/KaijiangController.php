...
class KaijiangController extends BaseController
{
    ...
    {
        $lastLottery             = CakeLottery::getLastest()->toArray();
        $details                 = $this->getCakeResult($lastLottery['opencode']);
        $lastLottery['opencode'] = explode(',', $lastLottery['opencode']);
        $lastLottery['details']  = $details;
        $lastLottery['code']     = 'cake';
        ...
        $unclearMoney = CakeOrder::getUnclearMoney($where);
     ...
     * @throws \think\exception\DbException
     */
    public function kjCake()
    {
        echo "cakeno\tstart\tkaijiang...\n";
        $start     = time();
        $lastest   = CakeLottery::getLastest();
        ...
        // 开奖结果
        $result = $this->cakeResult($open_code);
        // 兑奖
        ...
        $end    = time();
        $offset = $end - $start;
        echo "cakeno\t" . $msg . "\t" . "Lottery Successful\t[Time: {$offset} s]\n\n";
    }
    ...
    public function manualLottery()
    {
        ...
        return [
            'code' => 1,
            'msg'  => "加拿大28 " . $msg . " 开奖成功 [Time: " . $offset . " s]"
        ];
    }
     ...
     * @return array
     */
    private function cakeResult($open_code)
    {
    ...
    {
        $return = [];
        $lotteries = CakeLottery::getToday();
        $half_big_small = [];
        $half_even_odd = [];
        $color = [];
        foreach ($lotteries as $key => $lottery) {
            $result = $this->cakeResult($lottery->opencode);
            // 两面
