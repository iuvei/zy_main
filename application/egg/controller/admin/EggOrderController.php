...
class EggOrderController extends AdminBaseController
{
    /**
     * 用户下注历史
     *
    ...
    public function summaryMarkb(Request $request)
    {
        ...
        if (isset($param['expect'])) {
            if (!EggLottery::expectExists($param['expect'])){
                $this->jsonData['status'] = 0;
        ...
        // 查询所有有下注的玩法统计数据
        $markb = EggOrder::getMarkbOrders($where);
        // 查询所有玩法
    ...
    private function getAllMarkb()
    {
        ...
        $arr2 = ['红波', '绿波', '蓝波'];
        $odds = EggOdds::getRealOdds()->toArray();
        $newOdds = [];
